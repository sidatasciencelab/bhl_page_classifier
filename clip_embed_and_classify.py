...
def embed_and_classify_file_batch(file_list, output_file):
    ...
    batch_size = 32
    class_labels = ['A page of printed text', 
    ...
    with open(f'embeddings/{output_file}.npy','wb') as np_file:
        np.save(np_file, embeddings)

    prob_stack = np.vstack(class_probs)
    prob_df = pd.DataFrame(prob_stack)
    prob_df.columns = ['printed_text','handwritten_text','blank','cover','illustration','chart']
    prob_df['filename'] = [file.stem for file in file_list]
    prob_df.to_csv(f'probabilities/{output_file}.tsv', sep='\t', index=False)
...
if __name__ == "__main__":
    ...
    all_names = list(Path('page_thumbnails').rglob('*.jpg'))
